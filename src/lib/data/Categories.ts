...
export const CATEGORY_REGISTRY: ReadonlyMap<number, FixedCategory> = new Map([
  ...
  [20, { color: '#BBC200', ground: true, name: 'Festival Grounds', z_index: 2 }],
  ...
  [35, { color: '#8D8D8D', ground: true, name: 'Grounds', z_index: 1 }],
  ...
  [44, { color: '#CADC8C', ground: true, name: 'Light Green Camping Grounds', z_index: 0 }]
]);

export const CATEGORY_LIST = CATEGORY_REGISTRY.entries()
  .map(([fixed_id, data]) => ({
    fixed_id,
    ...data
  }))
